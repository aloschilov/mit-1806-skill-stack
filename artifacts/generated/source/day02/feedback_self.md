...
- Контрольный рубеж (gate) - проверяемый навык, который можно зачесть по решённой работе.
- Доказательство навыка (evidence) - конкретное решение, по которому видно, что навык освоен.
- Пространство столбцов \(C(A)\) (column space) - множество всех разрешимых правых частей \(b\).
- Нуль-пространство \(N(A)\) (nullspace) - множество решений однородной системы \(Ax=0\).
- Условие разрешимости (solvability condition) - критерий, по которому проверяется \(b\in C(A)\).
- Контрпример (counterexample) - короткий пример, который доказывает, что утверждение неверно.
