...
- Приведённая ступенчатая форма RREF (reduced row echelon form) - форма матрицы, из которой сразу читаются pivots и free variables.
- Опорный столбец (pivot column) - столбец с ведущей единицей в RREF.
- Опорная переменная (pivot variable) - неизвестная, соответствующая pivot column.
- Свободная переменная (free variable) - неизвестная, которую выбирают как параметр.
- Специальное решение (special solution) - базисный вектор нуль-пространства, построенный из одной выбранной free variable.
- Проверка rank-nullity (rank-nullity check) - равенство \(\dim N(A)=n-r\).
